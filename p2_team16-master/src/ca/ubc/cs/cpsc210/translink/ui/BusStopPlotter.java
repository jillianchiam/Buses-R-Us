...
public class BusStopPlotter extends MapViewOverlay {
        ...
        for (Stop s: StopManager.getInstance()) {
            if (Geometry.rectangleContainsPoint(northWest, southEast, s.getLocn())) {
                if (stopMarkerMap.containsKey(s)) {
                    Marker existingMarker = this.getMarker(s);
                    this.stopClusterer.add(existingMarker);

                } else {
                    makeNewMarkerAtCurrentLocation(currentLocation, s, stopIconDrawable);
                }
            }
        }
    }

    private void makeNewMarkerAtCurrentLocation(Location setMarker, Stop s, Drawable stopIconDrawable) {
        Marker makeMarker = new Marker(mapView);
        GeoPoint setPost = new GeoPoint(s.getLocn().getLatitude(), s.getLocn().getLongitude());
        makeMarker.setPosition(setPost);
        makeMarker.setTitle(s.getNumber() + " " + s.getName() + separateRoutesToOneLineEach(s));
        makeMarker.setIcon(stopIconDrawable);
        makeMarker.setRelatedObject(s);
        makeMarker.setInfoWindow(this.stopInfoWindow);
        this.setMarker(s, makeMarker);
        this.stopClusterer.add(makeMarker);

    }

    private String separateRoutesToOneLineEach(Stop s) {
    ...
    public void updateMarkerOfNearest(Stop nearest) {
        ...
        // unset the previous nearest station marker to be the green one
        if (nearestStnMarker != null) {
            nearestStnMarker.setIcon(stopIconDrawable);
        }

        // set the current nearest station marker to be green
