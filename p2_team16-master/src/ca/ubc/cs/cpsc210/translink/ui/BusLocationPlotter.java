    ...
    public void plotBuses() {
        ...
        if (StopManager.getInstance().getSelected() != null) {
            for (Bus b: StopManager.getInstance().getSelected().getBuses()) {
                OverlayItem overlay = new OverlayItem(b.getRoute().getName(), b.getDestination(),
                        new GeoPoint(b.getLatLon().getLatitude(), b.getLatLon().getLongitude()));
                busLocationsOverlay.addItem(overlay);
            }
        }
    }
